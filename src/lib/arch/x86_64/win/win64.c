...
char *strndup(char *s, int n)
{
        int len = strlen(s);
        int max = (len > n) ? n : len;
        char *all = malloc((max + 1)*sizeof(char));
        int i;
        for(i = 0; i < max; i++)
                all[i] = s[i];
        all[max] = 0;
...
static void *create_executable_code_x86_win64(char *machine_code, int size);
...
void *compile_function_internal(compiler c, token first_token, int *size)
{
        ...
        */
        ...
        void *executable = create_executable_code_x86_win64(code, i);
